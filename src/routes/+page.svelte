...
<div>
	<div class=" justify-center items-center">
		<h1 class="text-4xl text-center p-4">Movie Finder</h1>
		...
		<div class="flex flex-row gap-2 justify-center m-2">
			{#if searching}
				<button disabled type="button" class="rounded-md bg-teal-500 px-2 py-1 items-center flex">
					<svg
						aria-hidden="true"
						role="status"
			...
			{:else}
				<button
					class="rounded-md bg-teal-500 px-3 py-1 flex"
					disabled={searching}
					on:click={handleClick}>Go</button
				>
			{/if}

			<button class="rounded-md bg-blue-500 px-3 py-1 flex" on:click={getRandomPrompt}
