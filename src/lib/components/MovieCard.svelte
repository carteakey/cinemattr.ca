...
<div>
	...
	{:then data}
		{#if data.error}
			<div in:fade class="flex flex-col bg-neutral-800/70 shadow-md p-4 md:p-6">
				<div class="font-bold text-slate-200 text-md mb-2">{title_id}</div>
				<p class="text-slate-200/80 text-sm italic">{data.error}</p>
			</div>
		{:else}
			<div
				in:fade
				class="flex flex-col md:flex-row bg-neutral-800/70 shadow-md p-4 md:p-6 items-center"
			>
				<div
